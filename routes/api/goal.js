...
router.put("/like/:id", auth, async (req, res) => {
  try {
    ...
    if (
      goal.likes.filter(like => like.user.toString() === req.user.id).length > 0
    ) {
...
router.put("/unlike/:id", auth, async (req, res) => {
  try {
    ...
    // Check if the post has already been liked
    if (
      goal.likes.filter(like => like.user.toString() === req.user.id).length ===
      0
    ) {
...
// The route to the application is POST api/goal/comment/:id
// The description: Create goal comment
// Who can access this route. Private.
...
// The route to the application is DELETE api/goal/comment/:id/:comment_id
// The description: Delete Comment
// Who can access this route. Private.
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
    try {
        ...
        // Pull out comment
        const comment = goal.comments.find(comment => comment.id === req.params.comment_id)
        ...
        // Get remove index
        const removeIndex = goal.comments.map(comment => comment.user.toString()).indexOf(req.user.id)
